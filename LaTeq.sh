...
if [ $# -eq 0 ]; then
    echo "Usage: LaTeq \"equation\" [--png|--jpeg] [--output /path/to/dir] [--filename name] [--packages \"pkg1,pkg2,pkg3\"] [--dpi value]"
    echo "Example: LaTeq \"3x+1\""
    echo "Example: LaTeq \"3x+1\" --png"
    echo "Example: LaTeq \"3x+1\" --jpeg --output ~/Documents"
    echo "Example: LaTeq \"3x+1\" --png --output ~/Documents --filename \"my_equation\""
    echo "Example: LaTeq \"3x+1\" --png --dpi 300"
    echo "Example: LaTeq \"\\tikz \\draw (0,0) circle (1cm);\" --packages \"tikz\""
    echo "Example: LaTeq \"\\chemfig{H-C(-[2]H)(-[6]H)-H}\" --packages \"chemfig,xcolor\""
